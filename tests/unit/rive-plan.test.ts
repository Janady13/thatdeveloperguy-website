...
test('the lobby plan names three doors with hinges at the package pivots and hotspots that carry the site page ids', () => {
  ...
  assert.deepEqual(plan.doors.map(d => [d.id, d.hinge, d.trigger]), [['Door_IT', [564, 529], 'openDoor_IT'], ['Door_Gov', [804, 529], 'openDoor_Gov'], ['Door_Cyber', [1049, 529], 'openDoor_Cyber']]);
...
test('every door layer enters Closed and opens on its trigger; focus is driven from Any State', () => {
  ...
  assert.deepEqual(open.keys.slice(0, 2).map(k => [k.target, k.property, k.frame, k.value]), [['pivot:door_it_leaf', 'scaleX', 0, 100], ['pivot:door_it_leaf', 'scaleX', 26, 35]]);
