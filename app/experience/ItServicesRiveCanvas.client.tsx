...
/** Page hotspot ids → the room's `hoverTarget` numbers (TDG_IT_SERVICES_ANIMATION_KIT build-spec order). */
const HOVER_TARGET: Record<string, number> = { status: 1, helpdesk: 2, workstations: 3, racks: 4, deployment: 5, toolcart: 6 };
const PLATES = '/animation/rooms/it-services-plates';
const DOOR_MS = 650, DRAWER_MS = 450, DRAWER_STAGGER_MS = 90, LAPTOP_MS = 450, CHAIR_MS = 500;
...
/**
 * Native IT Services room on the clean plates (rive/it-services). HTML hit shapes own hover and navigation; this component
 * mirrors the focused hotspot into the room's view model (hoverTarget) and drives the kit's effects: display sweep on the
 * status board, door open on deployment, drawer cascade on the tool cart, laptop tilt on the racks, chair nudge on the workstations.
 */
export default function ItServicesRiveCanvas({ asset, focus, onReady, onError }: Props) {
  ...
  useEffect(() => {
    ...
    if (!artboard || !machines.includes(asset.stateMachine)) {
      callbacks.current.onError(new Error(`Incompatible IT Services Rive contract: ${asset.artboard} / ${asset.stateMachine}`));
      return;
    }
  ...
  useEffect(() => {
    ...
    if (!vmi) return;
    const number = (name: string) => { const property = vmi.number(name); if (!property) throw new Error(`IT Services view model has no number ${name}`); return property; };
    const target = focus ? HOVER_TARGET[focus] ?? 0 : 0;
    number('hoverTarget').value = target;
    if (focus === 'status') { const flag = vmi.boolean('refreshDisplay'); if (flag) { flag.value = true; setTimeout(() => { flag.value = false; }, 120); } }
    const goals: Array<[string, number, number, number]> = [
      ['doorAmount', focus === 'deployment' ? 1 : 0, DOOR_MS, 0],
      ['laptopAmount', focus === 'racks' ? 1 : 0, LAPTOP_MS, 0],
      ['chairAmount', focus === 'workstations' ? 1 : 0, CHAIR_MS, 0],
      ...[1, 2, 3, 4, 5].map((i): [string, number, number, number] => [`drawer${i}Amount`, focus === 'toolcart' ? 1 : 0, DRAWER_MS, (i - 1) * DRAWER_STAGGER_MS]),
    ];
  ...
  return (
    <div
      ref={container}
      className="scene-rive it-services-rive"
      aria-hidden="true"
