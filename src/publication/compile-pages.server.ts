...
import type { CompiledPage, PageRecord, PublicationStatus, RouteManifestEntry } from '../contracts/page.ts';
...
export function loadRecords(contentDir: string): Array<{ record: PageRecord; dir: string }> {
  ...
  return readdirSync(pagesDir).filter(name => existsSync(join(pagesDir, name, 'page.json'))).map(name => {
...
export function compilePages(contentDir: string, level: PublicationLevel): { pages: CompiledPage[]; manifest: RouteManifestEntry[]; drafts: string[] } {
  ...
  const pages: CompiledPage[] = live.map(({ record, dir }) => {
    ...
    return { ...record, bodyHtml, canonical: canonicalUrl(record.path), breadcrumbs, related, action: { label: record.primaryAction.label, href } };
  });
